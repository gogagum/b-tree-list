...
class _Allocator{
 private:
  ...
  const static size_t data_info_size = sizeof(_DataInfo);
...
{
  if (!new_file_flag) {
    *_data_info_ptr = *(reinterpret_cast<_DataInfo*>(_mapped_file_ptr->data()));
  } else { // file is new
...
template <typename __ElementType>
file_pos_t _Allocator<__ElementType>::NewNode() {
...
template <typename __ElementType>
void _Allocator<__ElementType>::DeleteNode(file_pos_t pos) {
...
template <typename __ElementType>
_Allocator<__ElementType>::~_Allocator() = default;
