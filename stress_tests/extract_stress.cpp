...
auto TestExtracts(size_t size_of_btree, size_t inserts_cnt) {
  ...
  for(auto i: positions) {
    test_list->Extract(i % test_list->Size());
  }
...
void RunExtractsTestPack() {
  ...
  for (size_t size_of_btree = step; size_of_btree *= step; size_of_btree < max_size + 1) {
    ...
    for (unsigned i = 0; i < 100; ++i) {
      ...
      file << TestExtracts(size_of_btree, inserts_cnt) / inserts_cnt << std::endl;
    }
    file.close();
  }
}
